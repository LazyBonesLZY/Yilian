...
- 登出失败会重试（最多 5 次，与 C 版一致）：没登出干净的会话会在 AC 那边挂着，占着账号的同时在线设备数
- 连接页展示会话细节：Algo-ID、Ticket、心跳新鲜度、校园网标志（wlanuserip 前两段）
...
- 前台服务常驻，可选开机自启
- **掉线检测与自动重连**：账号被限制多设备时会被静默踢下线，本客户端会主动发现并重新认证；检测间隔可自定义（10 秒 ~ 5 分钟）
- **防掉线**：心跳提前量、电池优化白名单入口、亮屏即探测
- 两档保活策略，在省电与及时性之间取舍
- 可把请求绑定到 Wi-Fi，避免误走蜂窝数据
- 日志页可调等级，可落盘、自动轮转、导出成文件
...
客户端有四重判定，任何一重不过就丢弃会话重新认证：

1. **心跳严格判定**——必须能解密、`code` 不是错误码、且带回 `interval` 节点。
   缺少 `interval` 即视为会话失效（与 C 版 `heartbeat()` 一致）。
   服务端**明确拒绝**（返回错误码或缺 `interval`）时立刻重认证，不再白等 3 次重试；
   只有网络不通这类可能自愈的失败才走「连续 3 次」。
2. **连通性探测**——按「掉线检测间隔」周期性请求外网地址，被重定向或收到门户页就说明已被踢。
3. **网络变化事件**——`ConnectivityManager.NetworkCallback` 一有动静立刻加探一次，不等周期到。
4. **屏幕点亮 / 解锁**——立刻加探一次。人从掏出手机到真正打开网页有几秒，
   足够把重认证跑完，多数掉线因此不会被察觉。
...
「设置 → 掉线重连」里用滑块自定义检测间隔，**10 秒 ~ 5 分钟**，5 秒一档，默认 45 秒。
间隔越短恢复越快、耗电也越多。省电优先模式下每次探测都要把设备从 Doze 唤醒，
因此最快按 45 秒执行——填得比这更短时界面会直接说明，想真正生效请切到稳定优先。

首页在发生过掉线后会显示自动重连次数与上次掉线时间。

## 防掉线

掉线本身拦不住——账号在别处登录是服务端的决定。能做的是别让**本可避免**的掉线发生：

**心跳提前量。** 服务端下发的 `interval` 是「最迟什么时候必须收到下一个心跳」，
掐着点发等于把网络延迟、闹钟迟到、Doze 唤醒延迟全押在零余量上：晚一秒，会话就被 AC 回收，
而客户端这边的心跳请求可能还显示成功。所以实际发送时间比它早 20%（下限 15 秒、上限 2 分钟）。
代价很小——10 分钟的间隔提前 2 分钟，一天也就多发三次心跳。

**电池优化白名单。** 这是最有效的一项，而且不在应用自己手里：只要还受电池优化管，
息屏后进程就会被冻进 Doze，非精确闹钟迟到几分钟很正常，心跳因此错过截止时间。
心跳提前量能扛住几十秒的迟到，扛不住「整个进程被冻住半小时」。
「设置 → 掉线重连 → 忽略电池优化」会显示当前状态，点一下跳系统授权；
个别 ROM 拦掉那个弹窗时会退回到电池优化列表页。

> 关于 `WifiLock`：查过了，**没有用**，所以没加。能在息屏时保住 Wi-Fi 的
> `WIFI_MODE_FULL_HIGH_PERF` 已被废弃，且会被系统自动替换成 `WIFI_MODE_FULL_LOW_LATENCY`，
> 而后者「仅在屏幕点亮且应用处于前台时生效」——正好和保活需要的场景相反。
> 加上它只会白耗电，还要多一项权限。
...
> 这两个通道已在校园网实测可用。解包链路另有逐字节回归测试：反向构造 9 个 ZSM 模块
> （oCode 1..9 各一个），用上游 C 实现取参考值，再用同一串字节走 Kotlin 实现，比对
> 模块 ID / 密钥 / IV / codex 与密文全链路一致（`DynamicZsmTest`）。
>
> `codex` ≥ 16（nCode）仍不支持，上游也没移植，遇到会明确报错。不同校区的 AC
> 下发的模块未必一样，认证失败请开 DEBUG 日志反馈。
...
界面上的日志只是内存里最近 800 条，进程被系统杀掉就没了——而掉线往往
恰好发生在半夜。所以默认同时写文件（设置 → 日志 → 记到文件，可关）：

- 位置：应用私有目录 `files/logs/run.log`，其他应用读不到，卸载即删
- 轮转：每 10000 行转成 `<时间戳>.rotate.log`，只保留最近 3 份
- 导出：日志页右上角「导出」，走系统的保存文件面板，自己挑存哪儿
- 「清空」会连日志文件一起删

轮转行数与 C 版 `utils/Logger.c` 一致；目录和保留份数不同——C 版写
`/var/log/esurfing` 且不清理旧文件，手机上两条都不合适。
...
| `core` | `DialerEngine` 状态机、`Timing` 心跳/探测节奏、`SettingsStore` 配置、`AppLog` 内存日志、`LogStore` 日志落盘 |
